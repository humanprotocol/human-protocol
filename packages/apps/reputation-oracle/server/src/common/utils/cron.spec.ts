...
describe('calculateBackoff', () => {
  it('should return 2 minutes backoff for the first retry (retriesCount = 0)', () => {
    ...
    expect(result).toBe(BACKOFF_INTERVAL_SECONDS * 1000); // 2 minutes in milliseconds
  });

  it('should return 4 minutes backoff for the second retry (retriesCount = 1)', () => {
    const retriesCount = 1;
    const result = calculateBackoff(retriesCount);
    expect(result).toBe(2 * BACKOFF_INTERVAL_SECONDS * 1000); // 4 minutes in milliseconds
  });

  it('should return 8 minutes backoff for the third retry (retriesCount = 2)', () => {
    const retriesCount = 2;
    const result = calculateBackoff(retriesCount);
    expect(result).toBe(4 * BACKOFF_INTERVAL_SECONDS * 1000); // 8 minutes in milliseconds
  });

  it('should return 64 minutes backoff for the sixth retry (retriesCount = 5)', () => {
    const retriesCount = 5;
    const result = calculateBackoff(retriesCount);
    expect(result).toBe(32 * BACKOFF_INTERVAL_SECONDS * 1000); // 64 minutes in milliseconds
  });
});
