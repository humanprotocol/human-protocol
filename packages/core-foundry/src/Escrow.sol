...
contract Escrow is IEscrow, ReentrancyGuard {
    ...
    function getBalance() public view returns (uint256) {
        (bool success, bytes memory returnData) =
            token.staticcall(abi.encodeWithSelector(FUNC_SELECTOR_BALANCE_OF, address(this)));
        if (success) {
    ...
    function addTrustedHandlers(address[] memory _handlers) public override trusted {
        _addTrustedHandlers(_handlers);
    }
    ...
    function setup(
        ...
        require(_reputationOracle != address(0), "Invalid reputation oracle address");
        require(_recordingOracle != address(0), "Invalid recording oracle address");
        require(_exchangeOracle != address(0), "Invalid exchange oracle address");
        uint256 _totalFeePercentage = uint256(_reputationOracleFeePercentage) + uint256(_recordingOracleFeePercentage)
    ...
    function cancel() public override trusted notBroke notComplete notPaid nonReentrant returns (bool) {
    ...
    function storeResults(string memory _url, string memory _hash)
        external
        override
        trustedOrRecordingOracle
        notExpired
    {
        require(
            status == EscrowStatuses.Pending || status == EscrowStatuses.Partial,
    ...
    function bulkPayOut(
        ...
        string memory _hash,
        uint256 _txId
    ) external override trustedOrReputationOracle notBroke notLaunched notPaid notExpired nonReentrant {
        ...
        require(status != EscrowStatuses.Complete && status != EscrowStatuses.Cancelled, "Invalid status");
        ...
        returns (uint256[] memory, uint256, uint256, uint256)
    {
        ...
        for (uint256 j; j < _amounts.length; j++) {
            ...
            {
                uint256 singleReputationOracleFee = uint256(reputationOracleFeePercentage).mul(amount).div(100);
                reputationOracleFee = reputationOracleFee.add(singleReputationOracleFee);
                amountFee = amountFee.add(singleReputationOracleFee);
            }

            {
                uint256 singleRecordingOracleFee = uint256(recordingOracleFeePercentage).mul(_amounts[j]).div(100);
                recordingOracleFee = recordingOracleFee.add(singleRecordingOracleFee);
                amountFee = amountFee.add(singleRecordingOracleFee);
            }

            {
                uint256 singleExchangeOracleFee = uint256(exchangeOracleFeePercentage).mul(_amounts[j]).div(100);
                exchangeOracleFee = exchangeOracleFee.add(singleExchangeOracleFee);
                amountFee = amountFee.add(singleExchangeOracleFee);
            }

            finalAmounts[j] = amount.sub(amountFee);
        }
        return (finalAmounts, reputationOracleFee, recordingOracleFee, exchangeOracleFee);
    }
    ...
    modifier trustedOrReputationOracle() {
        require(areTrustedHandlers[msg.sender] || msg.sender == reputationOracle, "Address calling not trusted");
        _;
    }

    modifier trustedOrRecordingOracle() {
        require(areTrustedHandlers[msg.sender] || msg.sender == recordingOracle, "Address calling not trusted");
        _;
    }
