...
contract RewardPool is IRewardPool, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _token, address _staking, uint256 _fees) external payable virtual initializer {
        __Ownable_init_unchained();

        __RewardPool_init_unchained(_token, _staking, _fees);
    }

    function __RewardPool_init_unchained(address _token, address _staking, uint256 _fees) internal onlyInitializing {
        token = _token;
        staking = _staking;
        fees = _fees;
    }
     ...
     */
    function addReward(address _escrowAddress, address _staker, address _slasher, uint256 _tokens)
        external
        override
        onlyStaking
    {
     ...
     */
    function getRewards(address _escrowAddress) external view override returns (Reward[] memory) {
