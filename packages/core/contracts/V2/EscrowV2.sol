...
import './interfaces/IEscrowV2.sol';
 ...
 */
contract EscrowV2 is IEscrowV2, ReentrancyGuard {
    using SafeERC20 for IERC20;

    string private constant ERROR_ZERO_ADDRESS = 'Escrow: zero address';
    uint32 private constant BULK_MAX_COUNT = 100;

    event TrustedHandlerAdded(address indexed handler);
    ...
    /**
     * @dev Finalizes the escrow, transferring remaining funds to the msg.sender
        ...
        nonReentrant
    {
        ...
        for (uint256 i; i < _recipients.length; ) {
            ...
            IERC20(token).safeTransfer(_recipients[i], netAmounts[i]);
            unchecked {
                ++i;
            }
        }
        ...
        if (exchangeOracleFeePercentage > 0) {
            IERC20(token).safeTransfer(exchangeOracle, totalExchangeOracleFee);
    ...
    modifier notComplete() {
        require(status != EscrowStatuses.Complete, 'Status Completed');
        _;
    }

    modifier notLaunched() {
        require(status != EscrowStatuses.Launched, 'Status Launched');
        _;
    }
