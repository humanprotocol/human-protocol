...
async function main() {
    ...
    const spokeAutomaticRelayerAddress = process.env.SPOKE_AUTOMATIC_RELAYER_ADDRESS;
    const magistrateAddress = process.env.MAGISTRATE_ADDRESS;
    const targetSecondsPerBlock = process.env.TARGET_SECONDS_PER_BLOCK;


    if (!governorAddress || !vHMTokenAddress || !spokeAutomaticRelayerAddress || !magistrateAddress) {
        throw new Error("One or more required environment variables are missing");
    }

    const DAOSpokeContractFactory = await ethers.getContractFactory("DAOSpokeContract");
    ...
    console.log("DAOSpokeContract deployed to:", await daoSpokeContract.getAddress());
}
