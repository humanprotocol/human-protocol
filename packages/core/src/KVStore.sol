...
contract KVStore {
    ...
    uint256 private constant BULK_MAX_COUNT = 20;
    ...
    function set(string memory _key, string memory _value) public {
        require(
            bytes(_key).length <= MAX_STRING_LENGTH && bytes(_value).length <= MAX_STRING_LENGTH,
    ...
    function setBulk(string[] memory _keys, string[] memory _values) public {
        require(_keys.length == _values.length, "Keys and values must have the same length");
