...
import { EscrowFactoryV2, HMToken, Staking } from '../../typechain-types';

describe('EscrowFactoryV2', function () {
  let owner: Signer,
    operator: Signer,
  ...
  let token: HMToken, escrowFactory: EscrowFactoryV2, staking: Staking;
  ...
  async function createEscrow() {
    ...
    const event = (
      result?.logs?.find(({ topics }) =>
        topics.includes(ethers.id('Launched(address,address,string)'))
      ) as EventLog
    )?.args;
  ...
  this.beforeEach(async () => {
    ...
    const EscrowFactory = await ethers.getContractFactory(
      'contracts/V2/EscrowFactoryV2.sol:EscrowFactoryV2'
    );
