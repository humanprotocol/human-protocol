...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_projects_cvat_cloudstorage_id'), 'projects', ['cvat_cloudstorage_id'], unique=False)
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('wait_until', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    ...
    sa.ForeignKeyConstraint(['user_wallet_address'], ['users.wallet_address'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
