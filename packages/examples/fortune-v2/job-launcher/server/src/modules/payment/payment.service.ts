...
import { Currency, PaymentFiatMethodType, PaymentSource, PaymentStatus, PaymentType, TokenId } from "../../common/enums/payment";
...
export class PaymentService {
  ...
  public async createFiatPayment(customerId: string, dto: PaymentFiatCreateDto) {
  ...
  public async confirmFiatPayment(userId: number, dto: PaymentFiatConfirmDto): Promise<boolean> {
    try {
      ...
      await this.savePayment(userId, PaymentSource.FIAT, PaymentType.DEPOSIT, BigNumber.from(paymentData.amount))
  ...
  public async createCryptoPayment(userId: number, dto: PaymentCryptoCreateDto) {
    try {
      ...
      if (transaction.confirmations < TX_CONFIRMATION_TRESHOLD) {
        this.logger.error(`Transaction has ${transaction.confirmations} confirmations instead of ${TX_CONFIRMATION_TRESHOLD}`)
        throw new NotFoundException(ErrorPayment.TransactionHasNotEnoughAmountOfConfirmations);
      } 

      await this.savePayment(userId, PaymentSource.CRYPTO, PaymentType.DEPOSIT, BigNumber.from(dto.amount));
  ...
  public async savePayment(userId: number, source: PaymentSource, type: PaymentType, amount: BigNumber): Promise<boolean> {
    const rate = await this.currencyService.getRate(TokenId.HUMAN_PROTOCOL, Currency.USD)

    const paymentEntity = await this.paymentRepository
      .create({
        userId,
