...
describe('Positive flow + adding same fortune. Only one unique fortune teller should be rewarded.', () => {
    test('Flow', async () => {
        ...
        for (let i = 0; i < agentAddresses.length; i++) {
            ...
            const agent_res = await sendFortune(agentAddresses[i], lastEscrowAddr, testFortune);
        ...
        expect(agent_1_balance-agentsOldBalances[0]).toBe(value - rewards.totalRecOracleReward - rewards.totalRepOracleReward);
