...
class TestEscrowClient(unittest.TestCase):
    ...
    def assert_wait_called_once_with(
        ...
        self.assertIs(args[0], self.escrow.w3)
        ...
        self.assertIs(args[3], EscrowClientError)
    ...
    def test_escrow_config_valid_params(self):
        ...
        manifest = "https://www.example.com/result"
        hash = "test"
        ...
        self.assertEqual(escrow_config.manifest, manifest)
        self.assertEqual(escrow_config.hash, hash)

    def test_escrow_config_valid_params_with_json_manifest(self):
        recording_oracle_address = "0x1234567890123456789012345678901234567890"
        reputation_oracle_address = "0x1234567890123456789012345678901234567890"
        exchange_oracle_address = "0x1234567890123456789012345678901234567890"
        manifest = '{"foo": "bar"}'
    ...
    def test_escrow_config_invalid_manifest(self):
        recording_oracle_address = "0x1234567890123456789012345678901234567890"
        reputation_oracle_address = "0x1234567890123456789012345678901234567890"
        exchange_oracle_address = "0x1234567890123456789012345678901234567890"
        manifest = ""
        ...
        self.assertEqual("Invalid empty manifest", str(cm.exception))
    ...
    def test_create_escrow_with_wait_options(self):
        escrow_address = "0x1234567890123456789012345678901234567890"
        token_address = "0x1234567890123456789012345678901234567890"
        job_requester_id = "job-requester"
        ...
        mock_create.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_create_fund_and_setup_escrow_with_wait_options(self):
        mock_create = MagicMock()
        mock_create.transact.return_value = "tx_hash"
        self.escrow.factory_contract.functions.createFundAndSetupEscrow = MagicMock(
            return_value=mock_create
    ...
    def test_get_manifest(self):
        ...
        manifest = '{"foo": "bar"}'
        mock_contract = MagicMock()
        mock_contract.functions.manifest.return_value.call.return_value = manifest
        self.escrow._get_escrow_contract = MagicMock(return_value=mock_contract)
        result = self.escrow.get_manifest(escrow_address)
        self.escrow._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.manifest.assert_called_once_with()
        mock_contract.functions.manifest.return_value.call.assert_called_once_with()
    ...
    def test_setup_with_wait_options(self):
        mock_contract = MagicMock()
        mock_setup = MagicMock()
        mock_setup.transact.return_value = "tx_hash"
        mock_contract.functions.setup = MagicMock(return_value=mock_setup)
        ...
        mock_setup.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_fund_with_wait_options(self):
        escrow_address = token_address = "0x1234567890123456789012345678901234567890"
        amount = 100
        tx_options = self._wait_tx_options()
        self.escrow.get_token_address = MagicMock(return_value=token_address)
        ...
        mock_transfer.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_store_results_with_wait_options(self):
        mock_contract = MagicMock()
        mock_store = MagicMock()
        mock_store.transact.return_value = "tx_hash"
        mock_contract.functions.storeResults = MagicMock(return_value=mock_store)
        ...
        self.escrow._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.storeResults.assert_called_once_with(url, hash)
        mock_store.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_store_results_with_funds_to_reserve_positive(self):
        mock_contract = MagicMock()
        mock_store = MagicMock()
        mock_store.transact.return_value = "tx_hash"
        mock_contract.functions.storeResults = MagicMock(return_value=mock_store)
        ...
        self.escrow.store_results(
            escrow_address, url, hash, funds_to_reserve=funds_to_reserve
        )
    ...
    def test_store_results_with_funds_to_reserve_zero_allows_empty(self):
        ...
        escrow_address = "0x1234567890123456789012345678901234567890"
        # Intentionally empty / invalid url & hash should PASS when funds_to_reserve == 0
        url = ""  # empty
        hash = ""  # empty
        funds_to_reserve = 0
    ...
    def test_store_results_with_funds_to_reserve_positive_invalid_url(self):
        escrow_address = "0x1234567890123456789012345678901234567890"
        url = "invalid_url"
        hash = "hash"
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.store_results(escrow_address, url, hash, funds_to_reserve=1)
        self.assertEqual(f"Invalid URL: {url}", str(cm.exception))

    def test_store_results_with_funds_to_reserve_positive_empty_hash(self):
        escrow_address = "0x1234567890123456789012345678901234567890"
        url = "https://www.example.com/result"
        hash = ""
        with self.assertRaises(EscrowClientError) as cm:
    ...
    def test_store_results_with_negative_funds_to_reserve(self):
        escrow_address = "0x1234567890123456789012345678901234567890"
        url = "https://www.example.com/result"
        hash = "hash"
        with self.assertRaises(EscrowClientError) as cm:
    ...
    def test_bulk_payout_with_wait_options(self):
        mock_contract = MagicMock()
        mock_bulk = MagicMock()
        mock_bulk.transact.return_value = "tx_hash"
        mock_contract.functions.bulkPayOut = MagicMock(return_value=mock_bulk)
        ...
        mock_bulk.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_complete_with_wait_options(self):
        mock_contract = MagicMock()
        mock_complete = MagicMock()
        mock_complete.transact.return_value = "tx_hash"
        mock_contract.functions.complete = MagicMock(return_value=mock_complete)
        ...
        self.escrow._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.complete.assert_called_once_with()
        mock_complete.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_request_cancellation_with_wait_options(self):
        escrow_address = "0x1234567890123456789012345678901234567890"
        mock_contract = MagicMock()
        mock_request = MagicMock()
        mock_request.transact.return_value = "tx_hash"
        ...
        mock_request.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_cancel_with_wait_options(self):
        escrow_address = "0x1234567890123456789012345678901234567890"
        tx_options = self._wait_tx_options()
        ...
        self.escrow._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.cancel.assert_called_once_with()
        mock_cancel.transact.assert_called_once_with({"gas": tx_options["gas"]})
        self.escrow.w3.eth.wait_for_transaction_receipt.assert_called_once_with(
            "tx_hash"
        )
        self.assert_wait_called_once_with(
    ...
    def test_get_remaining_funds(self):
        ...
        result = self.escrow.get_remaining_funds(escrow_address)

        self.escrow._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.remainingFunds.assert_called_once_with()
        self.assertEqual(result, 100)

    def test_get_remaining_funds_invalid_address(self):
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.get_remaining_funds("invalid_address")
        self.assertEqual(f"Invalid escrow address: invalid_address", str(cm.exception))

    def test_get_remaining_funds_invalid_escrow(self):
        self.escrow.factory_contract.functions.hasEscrow = MagicMock(return_value=False)
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.get_remaining_funds(
                "0x1234567890123456789012345678901234567890"
            )
        self.assertEqual(
            "Escrow address is not provided by the factory", str(cm.exception)
    ...
    def test_get_fund_amount(self):
        mock_contract = MagicMock()
        mock_contract.functions.fundAmount = MagicMock()
        mock_contract.functions.fundAmount.return_value.call.return_value = 100
        self.escrow._get_escrow_contract = MagicMock(return_value=mock_contract)
        escrow_address = "0x1234567890123456789012345678901234567890"

        result = self.escrow.get_fund_amount(escrow_address)

        self.escrow._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.fundAmount.assert_called_once_with()
        self.assertEqual(result, 100)

    def test_get_fund_amount_invalid_address(self):
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.get_fund_amount("invalid_address")
        self.assertEqual(f"Invalid escrow address: invalid_address", str(cm.exception))

    def test_get_fund_amount_invalid_escrow(self):
        self.escrow.factory_contract.functions.hasEscrow = MagicMock(return_value=False)
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.get_fund_amount("0x1234567890123456789012345678901234567890")
        self.assertEqual(
            "Escrow address is not provided by the factory", str(cm.exception)
        )
    ...
    def test_get_manifest_without_account(self):
        mock_provider = MagicMock(spec=HTTPProvider)
        w3 = Web3(mock_provider)
        mock_chain_id = ChainId.LOCALHOST.value
        type(w3.eth).chain_id = PropertyMock(return_value=mock_chain_id)
    ...
    def test_get_manifest_invalid_escrow(self):
        self.escrow.factory_contract.functions.hasEscrow = MagicMock(return_value=False)
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.get_manifest("0x1234567890123456789012345678901234567890")
        self.assertEqual(
            "Escrow address is not provided by the factory", str(cm.exception)
        )
    ...
    def test_get_intermediate_results_hash(self):
        mock_contract = MagicMock()
        mock_contract.functions.intermediateResultsHash = MagicMock()
        mock_contract.functions.intermediateResultsHash.return_value.call.return_value = (
            "mock_hash"
        )
        self.escrow._get_escrow_contract = MagicMock(return_value=mock_contract)
        escrow_address = "0x1234567890123456789012345678901234567890"

        result = self.escrow.get_intermediate_results_hash(escrow_address)
        self.escrow._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.intermediateResultsHash.assert_called_once_with()
        self.assertEqual(result, "mock_hash")

    def test_get_intermediate_results_hash_invalid_address(self):
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.get_intermediate_results_hash("invalid_address")
        self.assertEqual(f"Invalid escrow address: invalid_address", str(cm.exception))

    def test_get_intermediate_results_hash_without_account(self):
        mock_provider = MagicMock(spec=HTTPProvider)
        w3 = Web3(mock_provider)
        mock_chain_id = ChainId.LOCALHOST.value
        type(w3.eth).chain_id = PropertyMock(return_value=mock_chain_id)
        ...
        result = escrowClient.get_intermediate_results_hash(escrow_address)
        escrowClient._get_escrow_contract.assert_called_once_with(escrow_address)
        mock_contract.functions.intermediateResultsHash.assert_called_once_with()
        self.assertEqual(result, "mock_hash")

    def test_get_intermediate_results_hash_invalid_escrow(self):
        self.escrow.factory_contract.functions.hasEscrow = MagicMock(return_value=False)
        with self.assertRaises(EscrowClientError) as cm:
            self.escrow.get_intermediate_results_hash(
                "0x1234567890123456789012345678901234567890"
            )
        self.assertEqual(
            "Escrow address is not provided by the factory", str(cm.exception)
