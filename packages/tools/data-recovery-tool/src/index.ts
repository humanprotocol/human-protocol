...
const subgraphUrl =
  ...
  'http://localhost:8000/subgraphs/name/posix4e/humansubgraph';
const rpcUrl = process.env.RPC_URL || 'https://rpc.ankr.com/eth_rinkeby';
const factoryAddress =
  process.env.FACTORY_CONTRACT_ADDRESS ||
  '0x925B24444511c86F4d4E63141D8Be0A025E2dca4';
const network = process.env.NETWORK || 'rinkeby';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const gqlFetch = (url: string, query: string, variables?: any, headers?: any) =>
  fetch(url, {
    method: 'POST',
    headers: {
...
async function fetchEscrowEventsAsync() {
  const eventDayDatas = await gqlFetch(subgraphUrl, RAW_EVENT_DAY_DATA_QUERY)
    .then((res) => res.json())
    .then((json) =>
...
async function fetchEscrowStatsAsync() {
  const eventDayDatas = await gqlFetch(subgraphUrl, RAW_ESCROW_STATS_QUERY)
    .then((res) => res.json())
    .then((json) => {
      const {
        intermediateStorageEventCount,
...
async function fetchEscrowAmountsAsync() {
  ...
  fs.readFile(
    path.join(
      __dirname,
      `./abis/EscrowFactory_${network}_${factoryAddress}.json`
    ),
    'utf8',
...
function writeFile(data: any, name: string) {
  fs.writeFile(
    `./results/${network}_${factoryAddress}_${name}.json`,
    data,
    function (err) {
      if (err) {
        return console.log('error');
